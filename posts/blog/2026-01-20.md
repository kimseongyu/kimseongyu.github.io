...
1. 블로그 빌드 과정 중에 외부 GitHub 저장소의 프로젝트를 가져오는 방법
2. `sed` 명령어로 빌드 설정을 동적으로 수정하여 서브 경로(`basePath`) 문제 해결하기
3. 하나의 GitHub Pages 사이트에 여러 프로젝트를 통합하여 배포하는 파이프라인 구축
...
대신 **GitHub Actions**를 사용하면 배포 시점에만 외부 프로젝트를 가져와 빌드하고 합칠 수 있어 훨씬 깔끔하게 관리할 수 있습니다.
